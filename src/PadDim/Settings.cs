...
public sealed record Settings
{
    ...
    public int BrightnessPercent { get; init; } = 20;
    ...
    public static Settings Load()
    {
        ...
        return s with { IdleSeconds = Math.Clamp(s.IdleSeconds, 10, 86400), DimPercent = Math.Clamp(s.DimPercent, 5, 90), DeadzonePercent = Math.Clamp(s.DeadzonePercent, 1, 40), BrightnessPercent = Math.Clamp(s.BrightnessPercent, 0, 100), FadeSeconds = Math.Clamp(s.FadeSeconds, 0, 60) };
    }
    public void Save()
    {
